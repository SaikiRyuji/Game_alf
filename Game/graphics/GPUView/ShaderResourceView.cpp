...
bool CShaderResourceView::Create(CStructuredBuffer& structuredBuffer)
{
	...
	if (pBuf != nullptr) {
		...
		HRESULT hr = GfxEngine().GetD3DDevice()->CreateShaderResourceView(pBuf, &desc, &m_srv);
		if (FAILED(hr)) {
			return false;
		}
	}
...
bool CShaderResourceView::Create(ID3D11Texture2D* texture)
{
	...
	if (texture != nullptr) {
		...
		HRESULT hr = GfxEngine().GetD3DDevice()->CreateShaderResourceView(texture, &SRVDesc, &m_srv);
		if (FAILED(hr)) {
			return false;
		}
	}
...
bool CShaderResourceView::CreateFromDDSTextureFromFile(const wchar_t* fileName)
{
	...
	HRESULT hr = DirectX::CreateDDSTextureFromFileEx(
		GfxEngine().GetD3DDevice(), fileName, 0,
