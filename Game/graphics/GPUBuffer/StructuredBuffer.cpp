...
bool CStructuredBuffer::Create(const void* pInitData, D3D11_BUFFER_DESC& bufferDesc)
{
	...
	if (pInitData) {
		...
		hr = GfxEngine().GetD3DDevice()->CreateBuffer(&bufferDesc, &InitData, &m_structuredBuffer);
	}
	else {
		hr = GfxEngine().GetD3DDevice()->CreateBuffer(&bufferDesc, NULL, &m_structuredBuffer);
	}
	if (FAILED(hr)) {
		return false;
	}
