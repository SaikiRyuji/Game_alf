...
void Skeleton::InitBoneMatrixArrayStructuredBuffer()
{
	...
	//StructuredBufferを作成。
	GfxEngine().GetD3DDevice()->CreateBuffer(&desc, NULL, &m_boneMatrixSB);
}
void Skeleton::InitBoneMatrixArrayShaderResourceView()
{
	...
	GfxEngine().GetD3DDevice()->CreateShaderResourceView(m_boneMatrixSB, &desc, &m_boneMatrixSRV);
}
...
void Skeleton::SendBoneMatrixArrayToGPU()
{
	...
	//StructuredBufferを更新。
	GfxEngine().GetD3DDeviceContext()->UpdateSubresource(
		m_boneMatrixSB, 0, NULL, &m_boneMatrixs.front(), 0, 0
	);
	//ボーン行列を頂点シェーダーステージに設定。
	GfxEngine().GetD3DDeviceContext()->VSSetShaderResources(enSkinModelSRVReg_BoneMatrixArray, 1, &m_boneMatrixSRV);
